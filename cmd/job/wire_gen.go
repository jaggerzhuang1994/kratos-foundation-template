...
func wireApp(version app_info.Version, fileConfigSource conf.FileConfigSource) (*kratos.App, func(), error) {
	...
	cronjobJob := cronjob.NewJob(logLog)
	bootstrap := Boot(globalBootstrap, disableGrpc, logLog, register, cronjobJob)
	...
	contextContext, cleanup5, err := context.NewContext(contextHook, appInfo, logLog, v21, v18, metricsMetrics, tracingTracing)
	if err != nil {
		cleanup4()
		cleanup3()
		cleanup2()
	...
	kratosApp := app.NewApp(bootstrap, jobBootstrap, serverBootstrap, v19, appHook, appInfo, contextContext, logger, serverRegister, v21)
